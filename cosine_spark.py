import os, sys
import pandas as pd
import numpy as np
import math
from pyspark import SparkContext




def users(util_matrix):
	user_id = []
	for a in util_matrix:
		user_id.append(a)

	return set(user_id)




def cosinesim(util_matrix, u1, u2):
	movies = {}
	for movie in util_matrix[u1]:
		if movie in util_matrix[u2]:
			movies[movie] = 1
	length = len(movies)
	if length == 0:
		return 0

	sum_x = sum_xy = sum_xx = sum_y = sum_yy = 0

	for movie in movies:
		sum_x += util_matrix[u1][movie]
		sum_y += util_matrix[u2][movie]

		sum_xx += pow(util_matrix[u1][movie], 2)
		sum_yy += pow(util_matrix[u2][movie], 2)
		sum_xy += (util_matrix[u1][movie] * util_matrix[u2][movie])

	numerator = length * sum_xy - (sum_x * sum_y)

	denominator_square = (length * sum_xx - pow(sum_x, 2)) * (length * sum_yy - pow(sum_y, 2))
	denominator = pow(denominator_square, 0.5)

	if denominator == 0:
		return 0
	else:
		return numerator / denominator


# <codecell>

def getRecommendations(user, mov):
	totals = {}
	similarity_sums = {}
	rankings = []
	for other in users(utility):
		if other == user:
			continue
		similarity = cosinesim(utility, user, other)

		if similarity <= 0:
			continue
		# print("Similarity between " + str(user) + " and " + str(other) + " is " + str(similarity))
		for movie in utility[other]:

			if movie not in utility[user] or utility[user][movie] == 0:
				totals.setdefault(movie, 0)
				totals[movie] += utility[other][movie] * similarity
				similarity_sums.setdefault(movie, 0)
				similarity_sums[movie] += similarity

	rankings = [(total / similarity_sums[movie], movie) for movie, total in totals.items()]
	rankings.sort()
	rankings.reverse()
	topRankings = rankings[0:10]
	recommendationNumbers = [movie for score, movie in topRankings]
	recommemli = []

	for a in recommendationNumbers:
		recommemli.append((mov.take(mov.count())[a - 1][1]).encode('ascii', 'ignore'))
	return recommemli


# <codecell>

if __name__ == "__main__":

	sc = SparkContext.getOrCreate()
	ratingdf = sc.textFile("ml-latest-small/ratings.csv")
	ratingdfh = ratingdf.take(1)[0]

	ratings_data = ratingdf.filter(lambda line: line!=ratingdfh).map(lambda line: line.split(","))

	movies = ratings_data.map(lambda y: int(y[0]))
	user = ratings_data.map(lambda y: int(y[1]))
	rating = ratings_data.map(lambda y: float(y[2]))


	# ratings list will have [movieId, userId, rating]
	ratings_list = ratings_data.map(lambda x: (int(x[0]), int(x[1]), float(x[2])))
	print(ratings_list.collect())
	# read the movies file
	movieFile = sc.textFile("ml-latest-small/movies.csv")
	movieSplit = movieFile.map(lambda x: x.split(','))
	movie = movieSplit.map(lambda y: (y[0], y[2]))

	# generate user-movie-rating matrix.
	utility = {}
	for a in range(520):
		userId = user.take(user.count())[a]
		movieName = ratings_list.take(movie.count())[a][0]

		rate = rating.take(rating.count())[a]
		utility.setdefault(userId, {})
		utility[userId][movieName] = rate

	# User id of current user

	user_rec = 3
	inputId = user.filter(lambda a: a == user_rec).count()
	if inputId == 0:
		print("wrong user id")
	else:
		getrec = getRecommendations(user_rec, movie)
		if not getrec:
			print("No movies to recommend")
		else:
			print("Your recommended movies are" + '\n' + str(getrec))

	sc.stop()